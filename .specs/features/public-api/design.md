...
17. **`export * as X from './mod'` é proibido quando `./mod` reexporta um pacote externo via `export *` e tem outros exports** (descoberto na validation-engine T-001, ao expor `v`). `tsdown`/`rolldown-plugin-dts` 0.23.0/0.28.5 sintetiza um `declare namespace` pra materializar `X` nesse caso, e a sintetização descarta silenciosamente um dos dois lados (reexport externo ou exports locais — não é nem consistente qual lado sobrevive). Runtime sempre correto; só a declaração publicada quebra, sem erro de build. **Padrão obrigatório daqui pra frente para qualquer "reexportar um SDK externo como namespace membro de outro módulo"** (candidatos futuros: F09 se algum gerador OpenAPI externo for reexportado assim): declarar o valor como `const` tipado por interseção (`typeof pkg & typeof extras`) **e** um `namespace` de mesmo nome (merge valor+namespace, nativo do TS) só para os utilitários de tipo que precisem ser acessados como `X.algo<T>` — nunca `export * as X from`. Ver `src/validation/v.ts` (comentário no topo) para o exemplo de referência. Tipos auxiliares de retorno (não utilitários genéricos) ficam fora do namespace: colocá-los lá faz o bundler colapsar o alias num self-reference (`type Y = Y`, também reproduzido) quando o nome coincide com um tipo já hospedado no mesmo chunk — expô-los como export nomeado comum em vez disso.
