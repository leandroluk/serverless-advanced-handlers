...
**F05 `di-aot` concluída (3/3).** Primeiro código de compilador do projeto (usa `ts-morph`). T-001 (erros + detector de modo) e T-002 (analisador AST) em P1; T-003 (resolvedor de DI completo, `resolveAppGraph`) em P2, com 53 testes e fixtures usando decorators/tipos reais (sem stubs). QA reportou 3 defeitos menores não-bloqueadores na T-003: D-1 (token duplicado entre módulos não-`@Global()`: primeiro vence, silenciosamente) e D-2 (classe sem constructor próprio não herda deps da base) — ambos resolvidos com comentário de uma linha no código, sem mudança de comportamento; D-3 (marcadores de tipo Modo C disparam `SAH103` incorretamente) confirmado fora de escopo de T-003 e registrado como todo aberto pra REQ-007. `src/compiler/**` não é exportado por nenhuma entrada pública (só `/plugin` vai consumir, em F10) — sem impacto no snapshot de `public-api.spec.ts`.

Nenhuma feature em execução agora — F05 `di-aot` foi a última fechada. Próximo passo: decidir entre F06 `method-slicing` (depende de F05+F02, ambas concluídas) e F07 `http-runtime` (depende de F04+F05, ambas concluídas) como próximo item do roadmap.
...
- [x] Execute F05 T-001: `compiler/errors.ts` + `compiler/decorator-mode.ts` — PASS
- [x] Execute F05 T-002: `compiler/ast-analyzer.ts` — PASS
- [x] Execute F05 T-003: `compiler/di-resolver.ts` (algoritmo completo) — PASS. **F05 `di-aot` concluída (3/3).**
- [ ] Implementar REQ-007 (marcadores de tipo Modo C, ex. `Inject<typeof TOKEN>`) no resolvedor de DI — hoje `di-resolver.ts` só lê os decorators `@Inject`/`@Optional`, não os marcadores de tipo; um parâmetro Modo C sem decorator dispara `SAH103` incorretamente. Achado pelo QA na T-003 (D-3), confirmado fora do escopo de REQ-030..037. Bloqueia qualquer app que use decorators em Modo C (TC39 puro) com DI.

## Active Blockers
- none

## Recent Decisions (Last 15)
- [2026-09-16] F05 `di-aot` T-003: `AppGraph.providers` guarda todos os providers do app (não só `@Global()`); visibilidade de `@Global()` fica isolada num `globalRegistry` interno só usado no lookup de escopo. Controllers entram no grafo de detecção de ciclo (DFS cobre `graph.keys()`) mas ficam fora do array `order` final. Token duplicado entre módulos não-`@Global()` resolve pro primeiro varrido, silenciosamente (D-1, documentado com comentário) — decisão consciente, não lacuna; herança de constructor não é seguida (D-2, documentado com comentário) — limitação conhecida desta primeira versão.
...
- [2026-09-16] di-aot T-003 complete — algoritmo completo de `resolveAppGraph` (varredura de módulos, registro global, resolução de construtor, grafo de ciclo, ordenação topológica). Gate: 53/53 + `pnpm check`/`lint:ci`/`test`/`build`. QA PASS com 3 defeitos menores não-bloqueadores; PO ACCEPTED após dois comentários de documentação (D-1, D-2) adicionados diretamente pelo orquestrador (não pelo DEV, que foi interrompido por rate limit de sessão nesse meio-tempo). D-3 (marcadores de tipo Modo C) registrado como todo aberto pra REQ-007. **F05 `di-aot` concluída (3/3).** Commit: `feat(compiler)` di-resolver (este commit). [REQ-030..037]
- [2026-09-16] di-aot T-001 e T-002 complete (implementadas em paralelo, sem colisão). Gate: 17/17 + 25/25 + `pnpm check`/`lint:ci`/`test` (723/723)/`build`. QA PASS nas duas (gate reexecutado do zero, cenários adversariais próprios: ciclo de re-export circular, caso `getProviders()` sem validação, `Repo<User>` vs `Promise<Logger>`), PO ACCEPTED nas duas. `ts-morph@28.0.0` instalado (primeiro consumidor real). Commits: `feat(compiler)` errors+decorator-mode e `feat(compiler)` ast-analyzer (estes commits). [REQ-004, REQ-033, REQ-036]
...
- [2026-09-16] Ao retomar um agente DEV depois de um erro de rate limit de sessão (HTTP 429), não confiar no `result` parcial da notificação — o texto pode ser só a primeira frase antes do corte. Sempre confirmar direto no arquivo (grep/leitura) se a mudança pedida (mesmo pequena, como um comentário) realmente foi aplicada antes de seguir pro próximo passo.
